...
class ContainerResumeRunnerSpec extends ContainerResumeSpecBase {
    ...
    def "resuming a completed task prints the final status summary"() {
        given:
        repository.createTask(context('T-COMPLETED'), TaskStart.commit(cloneDir, 'HEAD'), TaskStart.pin('HEAD', BaseRule.LOCAL_HEAD), TaskState.atStageStart('build'))
    ...
    def "resuming an interrupted task with --discard-work disposes the kept environment"() {
        ...
        repository.createTask(context('T-DISC'), TaskStart.commit(cloneDir, 'HEAD'), TaskStart.pin('HEAD', BaseRule.LOCAL_HEAD), TaskState.atStageStart('build'))
    ...
    def "resuming a task with a pending interrupted verification leases the environment but skips salvage"() {
        ...
        repository.createTask(context('T-PEND'), TaskStart.commit(cloneDir, 'HEAD'), TaskStart.pin('HEAD', BaseRule.LOCAL_HEAD), TaskState.atStageStart('build'))
    ...
    def "resuming an interrupted task already at PipelineEnd touches no environment at all"() {
        ...
        repository.createTask(context('T-NOENV'), TaskStart.commit(cloneDir, 'HEAD'), TaskStart.pin('HEAD', BaseRule.LOCAL_HEAD), TaskState.atStageStart('build'))
