...
class SandboxLifecycleCrossInstanceE2ESpec extends Specification implements BareGitRepoFixture, AppAssemblyFixture {
    ...
    def "a sibling instance's claimed box survives this instance's sweep, and is stopped once the claim is gone"() {
        ...
        support.taskRepository().createTask(new TaskContext(taskId, UntrustedText.tracker('title'), UntrustedText.tracker('body'), List.<Decision> of()), TaskStart.commit(project, 'HEAD'), TaskStart.pin('HEAD', BaseRule.LOCAL_HEAD), TaskState.atStageStart('build'))
