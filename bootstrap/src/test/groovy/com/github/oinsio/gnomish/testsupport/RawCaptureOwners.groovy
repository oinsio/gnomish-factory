 ...
 *
 * <p>A file joining this map is the author stating which of design D3's seven families the text
 * belongs to. "Mechanics" and "factory-authored" are the two answers that are not a family: the
 * first is the deliberate carve-out for {@code :subprocess} and {@code :gitobjects}, whose contract
 * is process handling rather than text policy; the second is a document the factory itself wrote
 * and reads back, which is inside the boundary and mints nothing.
 */
class RawCaptureOwners {
    ...
    static final Map<String, String> CAPTURE_OWNERS = [
        ...
        'mechanics (D3 carve-out): drains a process into Captured, whose fields stay String so the '
        + 'module carries no text policy; every factory reader of a Captured mints at its own edge',
        'gitobjects/src/main/java/com/github/oinsio/gnomish/gitobjects/GitExec.java':
        'mechanics (D3 carve-out): the bare-object exec whose extraction-readiness depends on '
        + 'Captured staying String',
        ...
        'TRACKER (transport): holds the body as the cached/fresh envelope the tracker parsers read; '
        + 'the mint is at GithubTaskFetcher and GithubMarker, per D3',
        ...
        'MANIFEST/check (transport): streams the body under a byte cap into the check exchange; '
        + 'HttpExternalCheckClient matches it against the manifest\'s patterns and excerpts it '
        + 'through the carrier',
        ...
        'MANIFEST: reads a law file into Read.Text, whose unreadable arm already mints; the law text '
        + 'itself is a Control artifact the pipeline feeds to an executor, not prose a sink renders',
        ...
        'no mint owed: a secret value, which never reaches a log line or a comment by construction '
        + '(NFR-S1) — minting it would put it one accessor away from an exit',
        'application/src/main/java/com/github/oinsio/gnomish/dashboard/SnapshotReader.java':
        'no mint owed: factory-authored — reads back the snapshot this factory wrote; the untrusted '
        + 'fields inside it are re-minted by SnapshotJsonReader',
        ...
        'no mint owed: build-time only — reads this repository\'s own sources for the whole-tree '
        + 'gates. It is not on any runtime path, and the scan sees it because RepoSourceTree walks '
        + 'every src/main in the build, :test-fixtures included',
    ]
