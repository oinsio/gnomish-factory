...
class GitResumeBootstrapSpec extends GitResumeSpecBase {
    ...
    def "bootstrap() fast-forwards a worktree that is behind origin and discards uncommitted leftovers"() {
        ...
        repository().createTask(context('PROJ-20'), TaskStart.commit(cloneDir, 'HEAD'), TaskStart.pin('HEAD', BaseRule.LOCAL_HEAD), TaskState.atStageStart('implement'))
    ...
    def "bootstrap() leaves a worktree ahead of origin untouched"() {
        ...
        addRemote(cloneDir, 'origin', bare.toString())
        gitOutput(cloneDir, 'push', 'origin', 'HEAD:refs/heads/main')
        repository().createTask(context('PROJ-21'), TaskStart.commit(cloneDir, 'HEAD'), TaskStart.pin('HEAD', BaseRule.LOCAL_HEAD), TaskState.atStageStart('implement'))
    ...
    def "FR8: bootstrap() under a tenure discards a diverged local line and resumes from origin"() {
        ...
        addRemote(cloneDir, 'origin', bare.toString())
        gitOutput(cloneDir, 'push', 'origin', 'HEAD:refs/heads/main')
        repository().createTask(context('PROJ-22'), TaskStart.commit(cloneDir, 'HEAD'), TaskStart.pin('HEAD', BaseRule.LOCAL_HEAD), TaskState.atStageStart('implement'))
    ...
    def "FR8: bootstrap() with no claim on the task refuses a diverged branch instead of discarding it"() {
        ...
        addRemote(cloneDir, 'origin', bare.toString())
        gitOutput(cloneDir, 'push', 'origin', 'HEAD:refs/heads/main')
        repository().createTask(context('PROJ-23'), TaskStart.commit(cloneDir, 'HEAD'), TaskStart.pin('HEAD', BaseRule.LOCAL_HEAD), TaskState.atStageStart('implement'))
