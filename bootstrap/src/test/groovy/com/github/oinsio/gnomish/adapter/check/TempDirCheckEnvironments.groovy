 ...
 * requires an {@code AttemptCommitWorkspace} to know which commit to ask the platform about. In
 ...
 * AttemptCommitWorkspace} — so a mixed chain really is supported, but only over Docker. This stands
...
class TempDirCheckEnvironments implements CheckEnvironmentSource {
    ...
    @Override
    CheckEnvironmentSource.Acquired acquire(VerifyCheck.Command check, Workspace workspace) {
