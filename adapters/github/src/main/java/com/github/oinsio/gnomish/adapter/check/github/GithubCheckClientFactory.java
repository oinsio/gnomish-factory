 ...
 * and the token is resolved by name through the {@link SecretsProvider} the composition root
 * injects — in production the env/file adapter, backing {@link #TOKEN_ENV_VAR}. A
 ...
 * run's credential list, e.g. {@code RunAssembly.credentialNames}), so the variable can
