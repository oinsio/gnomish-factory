 ...
 * park/finish) — and carries back its comment id, {@code updated_at}, and
 * parsed marker so {@code heartbeat} (task 3.1), {@code listOpen} (3.2), and {@code
...
class GithubClaimCommentSpec extends Specification {
    ...
    private static String park(long id, String instance, String at, String reason = null) {
        markerComment(id, at, 'park', instance, at, reason)
    }
    ...
    def "FR5: a PARK (or FINISH) marker is a boundary that voids earlier claims"() {
        given:
        def json = array(
                claim(1, 'gnomish-factory-old', '2026-07-20T09:00:00Z'),
                park(2, 'gnomish-factory-old', '2026-07-20T11:00:00Z', 'checkpoint'),
    ...
    def "FR5: only the LATEST boundary anchors the window"() {
        ...
        def json = array(
                ...
                park(3, 'gnomish-factory-mid', '2026-07-20T10:00:00Z'),
                claim(920, 'gnomish-factory-fresh', '2026-07-24T10:00:00Z'))

        when:
        def resolved = GithubClaimComment.resolve(GithubClaimComment.parse(json))

        then: 'the mid claim (id 2) is before the latest boundary (the park); only the fresh claim survives'
