 ...
 *
 * <p>Capability siblings on the same DTO: {@link StageDtoSandboxSpec} (the sandbox
 * block, {@code verifyIn}, {@code pinPaths}) and {@link StageDtoProviderSpec} (the
 * external check's provider and its params).
 *
...
class StageDtoSpec extends Specification {
    ...
    def "the external check carries its identifier and raw timing strings"() {
        ...
        and: 'a manifest declaring no provider binds both provider keys as absent (FR13)'
