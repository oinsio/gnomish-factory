...
| `law-tamper-then-plain`       | multi-attempt: attempt 1 rewrites its own `.gnomish/instructions.md` control file in the workspace (the law root, D12 of add-base-ref-resolution) (a `TAMPERED LAW` `workspace-files/` overwrite) and produces no output, so a `files_exist` check fails and burns the attempt; attempt 2+ (same cwd) plays `plain-round` — the reward-hacking stand-in proving a mid-run law edit never re-enters the frozen law (FR19, NFR-S2, D14 of add-sandbox-core) |
