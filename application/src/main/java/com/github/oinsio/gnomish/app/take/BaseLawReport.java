 ...
 * and it leaves through the comment exit here — fenced and labeled, so the manifest's words are
 * visibly not the factory's (design D6, D10 of type-untrusted-text). The base ref beside them is not
 * untrusted text: every ref entering the process is held to {@code RefNameSyntax} first (NG4).
 *
...
public final class BaseLawReport {
    ...
    public static String of(String taskId, String baseRef, ObjectId lawCommit, List<ConfigError> errors) {
        String located =
                errors.stream().map(error -> error.render().forComment()).collect(Collectors.joining("\n"));
