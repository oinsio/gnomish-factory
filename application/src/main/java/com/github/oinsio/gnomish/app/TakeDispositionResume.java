 ...
 */
record TakeDispositionResume(
        TakeResumeRunner resumeRunner, TakeDecisionResume decisionResume, TaskGit git, Path worktreesRoot) {
    ...
    TakeResult resumeExisting(
        ...
        if (isOrphanedPark(bootstrap)) {
            return TakeReconcile.deliverPark(git, cloneDir, worktreesRoot, bootstrap, tracker, ref, instanceId);
        }
    ...
    static boolean isOrphanedPark(ResumeBootstrap bootstrap) {
        return bootstrap.trackerWritePending()
                && (bootstrap.outcome() instanceof RecordedOutcome.Escalated
                        || bootstrap.outcome() instanceof RecordedOutcome.Paused);
    }
