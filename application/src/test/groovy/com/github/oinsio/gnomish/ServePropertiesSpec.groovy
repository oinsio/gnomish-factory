...
class ServePropertiesSpec extends Specification {
    ...
    def "slots of #value is rejected with the property name in the message"() {
        ...
        where:
        //noinspection GroovyAssignabilityCheck -- Spock's where-block `<<` binds each list element to `value` across iterations, not a real leftShift call
        value << [-1, -100]
    }
    ...
    def "remote-probe-interval-cap of #value is rejected with the property name in the message"() {
        ...
        then:
        def failure = thrown(IllegalArgumentException)
        failure.message.contains('factory.serve.remote-probe-interval-cap')
    ...
    def "remote-sustained-open-threshold of #value is rejected with the property name in the message"() {
        ...
        then:
        def failure = thrown(IllegalArgumentException)
        failure.message.contains('factory.serve.remote-sustained-open-threshold')
