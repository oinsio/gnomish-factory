 ...
 *
 * {@code readRecordedState} is exercised end-to-end through {@link GitModeRunnerSpec}'s "removes the
 ...
 */
class RecordedStateReadbackSpec extends Specification {

    @TempDir
    Path tempDir

    def "readRecordedState() reads back the exact state.json content committed by the engine"() {
        ...
        when:
        TaskState read = TaskGitFixture.real().store().readRecordedState(worktree)
    ...
    def "readRecordedState() throws when state.json is missing"() {
        given:
        Path worktree = tempDir.resolve('empty-worktree')
        Files.createDirectories(worktree.resolve('.gnomish-task'))

        when:
        TaskGitFixture.real().store().readRecordedState(worktree)
