 ...
 *
 * <p>Class-level {@code @Timeout}: every feature here is in-memory and latch-driven, so a wait of
 * seconds means a slot permit was never returned — a deadlock, which is exactly what these
 * features exist to detect. Without the bound, the mutants that delete {@code SlotLedger.assign}
 * or {@code release} make the loop park forever and the covering feature hangs instead of
 * failing, which PIT reports as TIMED_OUT rather than KILLED (task 10.3 of split-into-modules;
 * same family as the source-level fixes task 9.1 made for `JudgePromptBuilder` and `GitExec`).
 * The bound is far above any real run and far below PIT's own per-mutation budget, so it turns a
 * hang into an ordinary failure without changing what any feature asserts.
 */
@Timeout(10)
