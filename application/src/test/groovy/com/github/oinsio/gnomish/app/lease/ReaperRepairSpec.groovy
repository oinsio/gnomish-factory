...
class ReaperRepairSpec extends Specification {
    ...
    def "a ready feed that fills the sweep page is logged"() {
        ...
        then:
        logged.any { it.formattedMessage.contains('filled the sweep page') }
    }
    ...
    //     can tell a race that converged from a repair that actually flipped labels.
    def "an unchanged repair logs the converging line, a repaired one does not"() {
        ...
        loggedOnUnchanged.any { it.formattedMessage.contains('already moved') }
