...
> **Status: walking skeleton, now self-feeding.** Requirements and architecture are shaped through [OpenSpec](openspec); the build, quality gates, and a minimal bootable application exist (see [Building](#building)). The domain core is in place — `.gnomish/` pipeline-config loading and the stage engine (a pure, reentrant orchestrator of the QC loop, driven entirely through ports). Its non-fake adapters have landed as a CLI that now runs unattended: [`gnomish run`](#running-a-task) drives a single task through the whole quality-control cycle, with real `agent-cli` and judge adapters by default and a git-backed task workflow — task branch, dedicated worktree, resume — as the normal mode; [`gnomish take`](#taking-a-task-from-a-tracker) layers a tracker-driven single-task workflow on top, with a GitHub adapter (plus an in-memory reference used for tests); and `gnomish serve` turns that into an autonomous daemon that feeds itself from the ready queue under a WIP-bounded, N-slot scheduler (see [`docs/operator-guide-serve.md`](docs/operator-guide-serve.md)). Several instances can share one project safely — claim leases with heartbeats, a reaper for dead claims, takeover, and zombie fencing — and the task lifecycle is enforced one-way (`Ready → Working → Finished`; a reopened finished task is declined, never re-claimed). Non-CLI AI-provider adapters and the sandboxed executor are not built yet.
...
The lifecycle is one-way — `Ready` → `Working` → `Finished`, never back. If a human reopens an already-finished task (relabeling or reopening the issue), the factory declines it instead of re-claiming it: it restores the terminal status and posts a comment pointing to a new task or bug, both via the polling feed and `gnomish take <ref>` run explicitly. See "Finished Tasks Are Terminal" in [`docs/operator-guide.md`](docs/operator-guide.md#finished-tasks-are-terminal).
