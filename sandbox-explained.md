...
# Трудные случаи (разобраны 2026-07-20)

## Гному нужен Docker внутри коробки (compose-стек для интеграционных тестов)

> Итоговая сводная лесенка Docker-стратегии (ступени 0–3, локально и в k8s,
> сквозные запреты) зафиксирована таблицей в `explore-notes-sandbox.md`,
> раздел «Итоговая Docker-стратегия» — **с пометкой обязательного ревью перед
> началом реализации** (инструменты быстро меняются).

Команда `docker` сама ничего не делает — она звонит СЛУЖБЕ Docker (демону) на
хосте. Дать коробке доступ к этой службе (пробросить сокет) = отдать ключи от
всей машины («запусти мне контейнер с полным доступом к диску хоста»);
запустить вторую службу внутри коробки = «привилегированный» режим, выключающий
изоляцию. Оба пути исключены.

**Ступень 0 (добавлена 2026-07-23): а нужен ли Docker именно в коробке?**
На стандартных GitHub-раннерах (`ubuntu-latest`) Docker и compose
предустановлены, Testcontainers работает из коробки, а раннер — одноразовая ВМ
на каждый прогон (изоляция — забота GitHub). Наш стадийный контракт уже
содержит нужный механизм — чек типа `external`: фабрика пушит ветку (и так
пушит после каждого раунда), CI гоняет интеграционную стадию, фабрика поллит
результат через трекер-порт. Для большинства проектов интеграционные тесты
выносятся в CI-чек — и весь узел ниже не активируется вовсе.
Предостережение в чеклист: ветка гнома = код гнома; workflow на push этой
ветки видит секреты репозитория — обманутый гном мог бы утащить CI-секрет
(GitHub сам ставит человеческий гейт «Approve and run workflows» своему
Copilot-агенту). Правило: workflow на `gnomish/*`-ветках — без привилегированных
секретов, `GITHUB_TOKEN` read-only (Testcontainers-тестам секреты и не нужны —
всё локально в раннере).

Если Docker всё же нужен в самой коробке — три честных пути:

1. **Сервисы-соседи от фабрики** (паттерн CI `services:`): статичный
   декларативный compose-стек (пример: Supabase для интеграционных тестов)
   поднимает ФАБРИКА снаружи, в той же внутренней сети задачи — гном видит
   хосты `db:5432`, `kong:8000` и гоняет тесты. Условие: compose из репо —
   недоверенный вход, фабрика фильтрует (запрет `privileged`, монтирований
   хоста вне рабочей копии, публикаций портов).
2. **Динамический Docker (Testcontainers-стиль: тест сам создаёт контейнеры
   на лету) в контейнерном режиме честно НЕ поддерживается** — нужна живая
   служба Docker. Такие проекты: доверенный режим на хосте либо путь 3.
   **Серединка для Linux-серверов — Sysbox** (проверено 2026-07: жив, открытая
   версия поддерживается; после покупки Docker'ом платная часть влилась в
   Docker Desktop как Enhanced Container Isolation). Это альтернативная среда
   запуска для обычного Docker: тот же `docker run` + флаг
   `--runtime=sysbox-runc`, внутри коробки становится можно запускать
   полноценную службу Docker БЕЗ привилегированного режима, изоляция даже
   строже обычной. Решение: в change A — ручка конфига
   `factory.sandbox.runtime` (дефолт — обычный runc): оператор Linux-сервера
   ставит пакет sysbox и включает; порт и адаптер не меняются; та же ручка
   потом включает gVisor. Фундаментом (обязательным) sysbox не делаем: на Mac
   его не поставить (ВМ Docker Desktop управляется самим Docker'ом) — поведение
   разъехалось бы между dev-Mac и сервером; плюс новый пререквизит хоста
   (пакет, свежее ядро) выше планки «Docker есть — всё работает».
3. **Мини-ВМ на задачу — настоящий ответ**: внутри собственной ВМ полноценная
   служба Docker легальна и безопасна (крутится на ядре задачи, хост не при
   делах) — так делают Claude Code on the web и Docker Sandboxes. Потребность
   в Docker внутри = главный триггер перехода с контейнер-адаптера на
   microVM-адаптер; непрозрачный порт такую замену предусматривает.

## Гному нужен внешний сервис (например, SonarQube)

1. **Для проверки — не нужен**: SonarQube-проверка в нашем стадийном контракте
   — `external`-чек, его опрашивает ФАБРИКА снаружи песочницы; токен сервиса в
   коробку не попадает вовсе. Уже так спроектировано.
2. **Если гному надо самому дотянуться** (прогнать sonar-scanner): одна строчка
   в списке вахтёра; работает и для интернета, и для локальной сети (вахтёр
   снаружи, сеть у него обычная).
3. **Два «но»**: токен сервиса — по правилу «ни одного настоящего секрета в
   коробке» (переложить на фабрику / временный узкий токен / с change B —
   впрыск вахтёром); владелец списка вахтёра — ОПЕРАТОР (конфиг фабрики), не
   файл в репо: иначе задача-обманка разрешит себе что угодно. Проект просит
   (в `.gnomish/`), оператор разрешает.

## Облачная коробка: ВМ в облаке или k8s (разобрано 2026-07-23)

Идея: запускать гнома в облачной ВМ (AWS/GCP/Azure/VK) или в k8s-поде, с
volume для файлов. Вердикт: **это третий адаптер того же непрозрачного порта**
(materialize / exec / harvest / dispose отображаются на облако один в один),
отдельный будущий change «cloud executor» — не A и не B.

- **Поправка про volume**: сохранность наработок УЖЕ решена глубже — фабрика
  после каждого раунда коммитит состояние в ветку и пушит; задача переживает
  гибель машины целиком, возобновляется любым инстансом (столп «stateless
  фабрика»). Переживающий падение volume — это КЭШ тёплого возобновления
  (не переклонировать, не перекачивать зависимости), не механизм сохранности.
  **Принцип: volume никогда не становится вторым источником правды** — сносим
  в любой момент без потери корректности.
- **Что покупает**: аппаратная изоляция (облачная ВМ = «мини-ВМ чужими
  руками», уровень недоверенных источников); эластичность (потолок — кошелёк,
  не железо); слой 2 вахтёра штатными средствами облака (Security Groups /
  NetworkPolicy); изоляция от домашней локальной сети оператора.
- **Что тянет**: обязательная блокировка metadata-сервиса облака
  (`169.254.169.254` — обманутый гном получил бы облачные креды; классическая
  грабля); exec/harvest становятся сетевыми (SSH / `kubectl exec`, git-daemon
  или bundle вместо «папки тома»); домены фильтрует всё равно наш прокси (SG —
  только IP) либо платные managed-фильтры; осиротевшая коробка = счёт в конце
  месяца (уборщик финансово критичен); данные уезжают в облако (новая точка
  доверия и юрисдикции).
- **k8s-first, сырые ВМ вторыми**: один API поверх всех облаков; pod-на-задачу,
  `kubectl exec`, PVC-кэш, ResourceLimits, NetworkPolicy default-deny — всё
  штатно; наши docker-образы и заготовки как есть; общее ядро ноды лечится
  runtimeClass (gVisor/Kata) — та же ручка runtime, что уже заложена; вложенный
  Docker болит так же (лесенка «трудных случаев» переносится).
- **Триггеры change'а**: не хватает железа под N слотов; наступил Д7
  (недоверенные источники → ВМ-изоляция); фабрика как сервис.
- **Testcontainers в k8s-подах** (разобрано 2026-07-23): проблема острее
  локальной — на нодах k8s Docker'а нет вообще (containerd), а классический
  CI-ответ (privileged DinD-сайдкар) ломает изоляцию и запрещён PodSecurity
  restricted; сокет ноды (CRI) в под — хуже docker.sock (вся нода). Лесенка:
  (1) сервисы-соседи переносятся даже лучше — namespace-на-задачу + стек
  обычными подами + NetworkPolicy default-deny в роли internal-сети;
  (2) динамический Testcontainers: **kubedock** первым (эмулятор Docker API
  поверх k8s: гному подсовывается DOCKER_HOST, поды создаются в namespace
  задачи под теми же политиками; без privileged; типовые сценарии работают,
  экзотика/bind-mounts болят; жив — 0.19.0 12.2025, принят в OpenShift Dev
  Spaces), **Sysbox RuntimeClass** вторым (dockerd в поде без privileged;
  нужен свой node pool), **Kata RuntimeClass** как настоящий ответ (под в
  микро-ВМ — DinD легален). Testcontainers Cloud — отвергнут (контейнеры
  тестов у третьей стороны, мимо вахтёра). Тонкость: образы в k8s тянет НОДА,
  не под — вне сетевой политики пода; registry-политика кластера — строка
  чеклиста.

**Архитектурное требование с сегодняшнего дня (решение оператора)**: фабрика
и гномы должны уметь запускаться и локально, и в k8s. Для change A это значит:
контракт порта окружения — **без предположения об общей файловой системе**
фабрики и коробки (пути томов — деталь локального адаптера; контракт говорит
языком git-транспорта и потоков), а сама фабрика остаётся честно stateless
(state = трекер + ветка) и конфигурируется извне — тогда её запуск в поде
не требует ничего нового.
...
## 3.4-доп. Готовые «коробочки» целиком (сверка ландшафта 2026-07-23)

За 2025–2026 вырос целый класс готовых инструментов «безопасная коробочка для
агента» — все они воспроизводят нашу же конструкцию (сеть по списку с отказом
по умолчанию, пустышки вместо ключей с впрыском на хосте, одноразовая среда на
задачу). Мы спроектировали мейнстрим; часть можно не строить, а взять.

| Класс | Представители | Суть | Для нас |
|---|---|---|---|
| Мини-ВМ «для агентов» | **Matchlock** (лучшее попадание: Firecracker на Linux + виртуализация Apple на Mac, default-deny сеть `--allow-host`, впрыск секретов вскрывающим прокси — наши шаги 3+4 из коробки), microsandbox (libkrun, <100 мс, Mac+Linux, SDK без Java), smolvm, agent-vm, drydock, CubeSandbox, bromure | граница-ядро на задачу + сеть + секреты одним инструментом | кандидаты на альтернативный адаптер порта |
| Контейнерные обвязки | mattolson/agent-sandbox (Docker+mitmproxy-сайдкар), hort (worktree+контейнер+allowlist одним бинарём), yolobox, OpenShell (NVIDIA) | наш же Docker, слои уже собраны | источник дизайна; сами слои у нас свои |
| Полная ВМ на проект | Tart (нативные ВМ Apple Silicon, образы через OCI-реестры), Lima-обвязки, nilbox | тяжелее, но максимум изоляции | запасной путь для Mac |

Почему default остаётся «Docker + свои слои»: (1) Docker уже пререквизит E2E и
управляется из фабрики программно, как git; (2) наша модель нестандартна для
этих инструментов — долгоживущая коробка на часы с запуском раундов внутрь,
сбор урожая из тома, заготовки-снимки, а «агентские» обвязки заточены под
one-shot «запустил — умерла»; (3) молодость и один-два мейнтейнера — риск для
фундамента, норма для адаптера.

**Matchlock — кандидат №1 на альтернативный адаптер** (впереди Apple container:
у того нет ни allowlist-сети, ни впрыска). Спайк-чеклист: долгоживущая ВМ +
exec раундов; том и сбор урожая; программное управление из JVM; аналог
заготовки; зрелость/bus-factor. Если спайк пройдёт — change B может взять
Matchlock вместо самосборного mitmproxy-вскрытия.
...
# Что ещё хочу обсудить

1. Можем ли сразу использовать Sysbox, чтобы обойти ограничение безопасного запуска Docker внутри Docker? Или ещё какой-нибудь аналог для локального запуска? Как дополнительный вариант запуск гнома в виртуальной машины в облаке (AWS, Google, Azure, VK-облако и т.п.), монтируя ему volume с файловой системой, который сможет пережить падение гнома и сохранить его наработки, сохранённые в Git? Или запуск на k8s в облаке? Может GitHub Actions поддерживает возможность запускать integration тесты с Testcontainers или docker-compose?
2. Как реализован безопасный запуск аналога гномов в проекте https://github.com/epam/dm.ai ? А что используют Firecracker или gVisor?
3. Как мы можем давать гному подключаться не только к Antropic, но и к другим AI-провайдерам? Например, на стадии написания кода гном пишет код с помощью Sonnet, а на стадии ревью или контроля качества будет проверять модель OpenAI?

> Статус (2026-07-31): вопрос 1 закрыт сессией ниже (Colima = отдельный
> вид коробки, облако и GHA — тоже; лесенка A–E). Вопрос 2 разобран в
> сессии; в заметки не выносим (референсом по изоляции проект не является).
> Вопрос 3 закрыт — см. «Докопали в тот же день» ниже.

---

# Сессия 2026-07-31: коробки разных видов и кому что можно

Разобрали оба исследования (эту тетрадь и `colima-agent-sandbox.md`) вместе
и приняли решения. Технические детали — в `explore-notes-sandbox.md`,
раздел «Сессия 2026-07-31»; угрозы №32–44 — в `sandbox-threats.md`.

## Семья коробок вместо одной

Коробка бывает разной, и это не конкуренция, а линейка сменных «моторов» под
одним и тем же капотом (порт один, адаптеры разные):

- **Docker-контейнер** — лёгкий и быстрый, основной вариант (change A);
- **Colima VM** — полноценная виртуальная машина на задачу, когда нужна
  железная граница локально (change C; исследование уже готово);
- **Облако (k8s)** — когда не хватает своей машины (change D);
- **GitHub Actions** — «облако без хозяйства», строим со всеми оговорками:
  у него самый слабый контроль сети, зато свои плюсы (одноразовая VM на прогон,
  секреты через штатные механизмы GitHub) — честный паспорт рисков (change E);
- **Хост без коробки** — остаётся навсегда для доверенного окружения и особых
  случаев (сборка под iOS, GPU); включить может только оператор.

Вахтёром с первого дня берём mitmproxy (в простом режиме, без вскрытия) —
чтобы потом не менять инструмент.

## Кто что решает: репо просит — оператор разрешает — фабрика сверяет

- **Проект (репо)** пишет, что его стадиям НУЖНО (Docker внутри, память,
  адреса, «мне нужна свежая коробка») — и может только УЖЕСТОЧАТЬ требования;
- **Оператор фабрики** решает, какой вид коробки какой стадии выдать — он
  единственный, кто может ослаблять (включая «запускай на хосте»);
- **Фабрика** сверяет «что нужно» с «что умеет выданная коробка» (у каждого
  вида — честный паспорт возможностей) и при несовпадении НЕ запускает
  задачу — «при сомнении — закрыто».

Иначе задача-обманка попросила бы себе коробку без стен.

## Коробка на стадию, а не на задачу

Разным стадиям — разные коробки: писать код можно в лёгкой, гонять тяжёлые
тесты — в облачной. Переезд между стадиями бесплатный по построению: состояние
задачи и так живёт в git-ветке, значит «собрать урожай → снести коробку →
развернуть новую из ветки» — это ровно та же механика, что возобновление
задачи другим инстансом. Соседние стадии с одинаковой коробкой фабрика
объединяет — но есть ручка «каждой стадии — свежую» (см. ниже).

## Две ручки свежести

- **Переиспользовать коробку между стадиями?** По умолчанию да (тёплые кэши —
  это минуты). Но стадия может потребовать свежую. Зачем: за время работы гном
  мог «отравить» коробку ВНЕ рабочей копии (автоскрипты Gradle, подмены в
  PATH) — в diff этого не видно, а свежая коробка сбрасывает всё.
- **Где гонять проверки (`command`-чеки)?** По умолчанию — в той же коробке
  (быстро). Для важных ворот качества — в свежей, развёрнутой ИЗ КОММИТА
  попытки: заодно доказывает, что гном ничего не забыл закоммитить —
  проверяется ровно то, что человек будет вливать.

## Данные опасны только там, где их слушаются (метод стоков)

Разобрали judge- и external-проверки по шагам. Вывод-метод: злой текст в
ответе судьи или в логах CI сам по себе не страшен — страшен только СТОК,
где этот текст кто-то исполнит или интерпретирует. Поэтому: фабрика слушается
только поля «прошло/не прошло», всё остальное — непрозрачный текст, который
чистится перед логами, публикуется в трекер с пометкой «машинный вывод», а
если попадает в промпт следующей попытки — падает в клетку, которая и так
построена в расчёте на обманутого гнома. Все findings идут через ОДНУ воронку,
где эта чистка написана и оттестирована один раз.

Два спутника этого разбора:

- **Гном может переписать сам CI-workflow в своей ветке** и «поставить себе
  зачёт» — поэтому первым чеком идёт дешёвая сверка: файлы-определения чеков
  в ветке гнома не отличаются от основной ветки; отличаются — сразу незачёт.
- **Применение ответов модели** (когда фабрика сама пишет файлы по указке
  модели): только внутри рабочей копии, никогда в `.git/**`, и недоверенные
  ветки никогда не разворачиваются в файлы на машине фабрики — только внутри
  коробки. Фабричный git никогда не исполняет хуки.

## Склад библиотек (Artifactory-класс)

Гном больше не качает библиотеки из интернета сам: между ним и миром — свой
склад (кандидат — Sonatype Nexus OSS: Maven+npm+PyPI+Docker одним сервисом).
Что это даёт:

- список вахтёра схлопывается до трёх адресов: шлюз, склад, git-сервер;
- утечка данных «закодирую в имени запрашиваемого пакета» умирает — гном
  говорит только со складом, наружу ходит склад;
- **карантин свежих версий** (отдавать пакеты старше N дней) — главное
  лекарство от атак типа s1ngularity: заражения обычно ловят за считаные дни;
- полный журнал «что качала задача» — аномалия видна в отчёте.

Держит защиту вахтёр (прямых реестров в его списке просто нет), а настройки
сборки на склад — удобство. Просьба проекта о экзотическом реестре — это
новый источник У СКЛАДА (с тем же карантином), а не дырка у вахтёра.
В change A — задел: адреса реестров в образе задавать параметром, не намертво.

## Сейф для ключей (Vault-класс)

Своим ключам (токен трекера, настоящий AI-ключ) — отдельный порт «поставщик
секретов»: по умолчанию простые env/файлы (ноль хозяйства), позже — сейф
класса Vault (смотреть OpenBao — открытый форк). Бюджеты на задачу остаются
у шлюза (сейф деньги не считает). Настоящая выгода сейфа — в облаке и GHA:
раннер/под получает короткоживущий ключ по OIDC, и статических секретов не
остаётся нигде.

## Докопали в тот же день (вторая итерация 2026-07-31)

- **Список вахтёра для коробки — всего два адреса: шлюз и склад.** Оказалось,
  git-сервер гному не нужен вовсе: копию проекта фабрика кладёт в коробку из
  своего локального клона (без сети), результат забирает сама. Дверь «чужой
  репозиторий на github.com» для коробки закрыта целиком.
- **Лимит денег на временном ключе — это уже и есть лимит объёма** (токены =
  байты): отдельного счётчика не нужно, добавляем только ограничение частоты
  запросов на шлюзе. Необычный расход (в разы больше типового для стадии) —
  сигнал в отчёте, как отказы вахтёра.
- **Новая найденная дыра: модель умеет сама ходить в интернет.** У провайдера
  есть «серверные инструменты» (веб-поиск, чтение страниц): обманутый гном
  просит модель «сходи по такому-то адресу с такими-то данными» — и запрос
  делает провайдер, мимо вахтёра и склада. Закрытие дешёвое: гном говорит со
  шлюзом открытым текстом, значит шлюз может вырезать такие инструменты из
  запросов без всякого вскрытия — если стадия явно не разрешила. (Угроза №45.)
- **Подготовка коробки упростилась.** Со складом установочной фазе не нужна
  «сеть пошире»: склад проксирует всё, включая установщики. А секреты
  установки исчезают по построению: доступ к приватному складу — это настройка
  НА СКЛАДЕ, в коробку такой секрет не попадает никогда. От «двух фаз» остаётся
  только правило «снимок заготовки делаем до первого касания гномом».
- **Разные модели на разных стадиях — получается бесплатно из шлюза**: стадия
  объявляет модель, временный ключ ограничен этой моделью, шлюз переводит
  протоколы (гном говорит на языке Anthropic, ревью может делать модель
  OpenAI). Проверить осталось только качество такой трансляции — спайком.
