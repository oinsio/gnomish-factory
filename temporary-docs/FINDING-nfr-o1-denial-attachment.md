...
| Fact | Location |
|------|----------|
| `denialFindings()` produces `List<Finding>` from the guard log | `sandbox/environment/EgressGuard.java:128` |
| `GuardDenialLog` parses marked JSONL into findings | `sandbox/environment/GuardDenialLog.java` |
| Denial read-back accessor exists only on the concrete type | `sandbox/environment/SelfCheckedEnvironment.java:78` |
| Port declares **no** denial accessor (8 methods, none egress) | `sandbox/environment/TaskExecutionEnvironment.java` |
