...
public interface Tracker {
     ...
     * in posting order. Empty means no new reply since the last {@link
     * #acknowledgeDecision(TaskRef, String)} — a stale reply must never resurface
     * after an ack (FR12).
     ...
     * {@code listReady} observes the updated {@link AbortFacts} (NFR-R3). Never
     * applies backoff or the K-abort fuse — that is core's job over these facts (D10).
    ...
    /**
     * Persists a structural durable-progress marker for {@code ref}, leaving its
     * logical state and claim holder untouched. Reconstructable from the tracker
     * alone: another instance then observes {@link AbortFacts} reset to "aborts
     * since this marker" (FR1, FR4, D1). Idempotent within a claim: a second marker
     * is harmless since reconstruction anchors to the latest (NFR-R2).
    ...
    /**
     * Posts an "acting on decision" marker naming {@code decisionText}, so a later
     * {@link #collectDecisions(TaskRef)} is empty until a new human reply arrives —
     * the single mechanism that both records the acted-on reply and anchors future
     * collection (FR12, UX3).
     * <p>Implements FR12 of add-tracker-port.
     * @param ref the task's canonical identity; never null
     ...
     * state — the general-purpose correspondence op for anything that is neither a
     * park report, finish summary, nor decision ack (NFR-O1).
     * <p>Implements FR1 of add-tracker-port.
     * @param ref the task's canonical identity; never null
    ...
    /**
     * Returns a stale claim to circulation as one operation, given {@code ref} and
     * the {@code observedVersion} judged stale: records a structural holder-transition
     * marker naming the dead holder, removes the dead marker, and transitions {@code
     * ref} back to {@link TrackerTaskState.Ready}. Never claims for the caller (design
     * D5, D9, FR4). {@code observedVersion} is a guard: if it no longer matches the
     * live claim, the call is a safe no-op returning {@link
     * RemoveStaleClaimResult.Mismatch} with the current version (NFR-R2); a match
     * yields {@link RemoveStaleClaimResult.Removed}. The marker SHALL be
