 ...
 * ManualRunAssembly#assemble}, but drives {@link Engine#run} directly instead of {@link
 ...
 * @param runner the git subprocess runner shared with the caller's bootstrap/salvage steps;
 *     never null
 * @param cloneDir the project clone the task worktree was materialized under; never null
 * @param worktreesRoot the worktrees root the task's {@link GitTaskRepository} is rooted
 *     under; never null
...
record TakeEngineExecution(
    ...
    TakeResult run(
        ...
        if (revocation.isPresent()) {
            ...
            var worktreeSalvage = new WorktreeSalvage(runner, worktree);
            var branchPush = new BranchPush(runner);
            var handler = new RevocationHandler(tracker, worktreeSalvage, branchPush);
