 ...
 *
 * <p>The package is a library by construction: it imports only the JDK and the SLF4J API — never
 * adapter, app, domain, status, usage, Spring, or Jackson types, nor any factory-root type (its PIT
 * marker is the library-local {@link com.github.oinsio.gnomish.gitobjects.DoNotMutate}), all pinned
 * by {@code GitObjectsBoundarySpec} — and every commit id is deterministic for fixed inputs because
