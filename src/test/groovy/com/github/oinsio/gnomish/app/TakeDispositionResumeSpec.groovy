...
/**
 * TakeDispositionResume.isEscalationDecision (FR9, D3 of add-tracker-port; FR9, D12): the routing
 ...
 */
class TakeDispositionResumeSpec extends Specification {

    private static TaskOutcomeDto escalated() {
        new TaskOutcomeDto.Escalated('escalated', new EscalationReportDto.AttemptsExhausted('attemptsExhausted', 3))
    }

    private static TaskOutcomeDto paused() {
        new TaskOutcomeDto.Paused('paused', 'build')
    }

    private static ResumeBootstrap bootstrap(TaskOutcomeDto outcome, boolean pending) {
        new ResumeBootstrap(
                'PROJ-1',
