 ...
 * stage — its {@code round} number, the {@code checkResults} it produced, the
 * round's {@code executorUsage} and its {@code judgeUsage} (design D4). The raw
 * ToolTrace is deliberately NOT held here; it lives outside TaskState, correlated
 * by AttemptKey (design D5). Implements FR13 of add-stage-engine.
 */
class AttemptRecordSpec extends Specification {
    ...
    // FR13: a record exposes its round, result, checkResults, executorUsage and judgeUsage as constructed
    def "exposes round, result, checkResults, executorUsage and judgeUsage as constructed"() {
        ...
        def record = new AttemptRecord(2, AttemptRecord.Result.PASSED, [check], executorUsage, judgeUsage)
    ...
    def "exposes the constructed result classification"() {
        ...
        new AttemptRecord(0, result, [passResult()], ExecutorUsage.none(), JudgeUsage.none()).result() == result
    ...
    def "a validated round round-trips the constructed literal"() {
        ...
        new AttemptRecord(6, AttemptRecord.Result.PASSED, [passResult()], ExecutorUsage.none(), JudgeUsage.none())
        .round() == 6
    }
    ...
    def "accepts round zero"() {
        ...
        def record = new AttemptRecord(0, AttemptRecord.Result.PASSED, [passResult()], ExecutorUsage.none(),
        JudgeUsage.none())
    ...
    def "rejects a negative round with the component named"() {
        ...
        new AttemptRecord(round, AttemptRecord.Result.PASSED, [passResult()], ExecutorUsage.none(), JudgeUsage.none())
    ...
    def "accepts an empty checkResults list"() {
        ...
        def record = new AttemptRecord(1, AttemptRecord.Result.DECISION_NEEDED, [], ExecutorUsage.none(),
        JudgeUsage.none())
    ...
    def "defensively copies checkResults so source mutation does not leak"() {
        ...
        def record = new AttemptRecord(0, AttemptRecord.Result.PASSED, source, ExecutorUsage.none(), JudgeUsage.none())
    ...
    // FR13: AttemptRecord is inert value data compared by content
    def "is value-equal by content"() {
        ...
        new AttemptRecord(1, AttemptRecord.Result.PASSED, [check], executorUsage, judgeUsage) ==
        new AttemptRecord(1, AttemptRecord.Result.PASSED, [check], executorUsage, judgeUsage)

        and: 'a differing round makes them unequal'
        new AttemptRecord(1, AttemptRecord.Result.PASSED, [check], executorUsage, judgeUsage) !=
        new AttemptRecord(2, AttemptRecord.Result.PASSED, [check], executorUsage, judgeUsage)

        and: 'a differing result classification makes them unequal (result is part of value identity)'
        new AttemptRecord(1, AttemptRecord.Result.PASSED, [check], executorUsage, judgeUsage) !=
        new AttemptRecord(1, AttemptRecord.Result.QUALITY_FAILURE, [check], executorUsage, judgeUsage)
    }
}
