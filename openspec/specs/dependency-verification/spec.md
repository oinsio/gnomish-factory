...
Gradle dependency verification pins every resolved artifact by checksum, so
an unexpected or tampered jar never reaches the JVM. It closes the build-time
trust gap left by the post-SecurityManager JDK (JEP 486) and hardens the
whole supply chain, complementing the existing dependency-locking flow
(`./gradlew check --write-locks`) that pins *which versions* resolve with a
mechanism that pins *which bytes* those versions are.
