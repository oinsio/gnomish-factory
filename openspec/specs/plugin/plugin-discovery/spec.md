...
Resolve every provider factory — tracker and check — through Java `ServiceLoader` instead of a hardwired registry, with the discovery mechanism identical for built-in and external providers. SPI factories construct with a public no-arg constructor and receive `SecretsProvider` and resolved config as method arguments; registries are per-port and selection by discriminator is independent per port so vendors may be mixed; resolution is deterministic and fails fast on a missing or duplicated discriminator; and the loaded provider set is reported at startup. The capability also defines the four-point plugin-ready criterion applied per port.
