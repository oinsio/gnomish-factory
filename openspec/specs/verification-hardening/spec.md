...
Verification hardening protects stage verdicts and their downstream findings from manipulation by untrusted gnome output. A unified findings funnel routes judge, external, and command-check output through one schema-validated, size-capped, sanitized pipeline before any sink, so factory control flow depends only on the structured verdict or exit code and findings text stays inert data everywhere — in logs, in tracker publication, and inside judge prompts, where artifact content is wrapped in hard delimiters. Sandboxed judge votes run in a fresh environment materialized from the attempt commit so a gnome-poisoned round box can never grade itself. A pin-check guard wraps every external-check adapter, refusing to invoke it if the check's own definition files have drifted from the base branch, and model-produced file writes are confined to the working copy with no path escaping it or `.git/`.
