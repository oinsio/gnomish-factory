...
Ship a built-in `provider: http` external check in core — the escape hatch for third-party CI and quality services reachable over HTTPS, symmetric to the `command` check for local verification. Its verdict is declarative (`pass-when`, optionally narrowed by a JSONPath-subset or regex extraction compared with `equals`) with optional `pending-when` polling over the existing `External` poll loop; it contributes no adapter pin paths, and any authorization credential is named in the manifest and resolved through `SecretsProvider` at request time.
