...
Define the `gnomish serve` daemon's file-based observability contract: a
**snapshot** — an atomically overwritten JSON gauge file answering "alive?
what are the slots doing?" — and a **ledger** — an append-only JSONL history
of terminal task outcomes and daemon lifecycle events, with daily UTC
rotation and a retention sweep. The daemon has no inbound HTTP and a tight
tracker write budget, so it must publish state and history as local files
instead of serving a dashboard or spending tracker calls on status.
