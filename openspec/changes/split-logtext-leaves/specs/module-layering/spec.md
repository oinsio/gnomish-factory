# module-layering — delta for split-logtext-leaves

Layered on the module tree as modified by `add-base-ref-resolution`
(sequenced before this change), which is itself layered on
`add-subprocess-access-log`: both requirements below are written over the
base-ref delta's text, so syncing in that order merges cleanly. If this
change syncs first, the two earlier syncs must merge by hand — the
`:baseref` sentences and scenario, the `:logtext` access-log sentences and
the `:gitobjects` observer-hook sentence and scenario — instead of replacing
the requirement.
