...
- [ ] 2.1 In `build-logic/build.gradle`, register a `functionalTest` JVM test suite (Spock + `gradleTestKit()`), add it to `gradlePlugin.testSourceSets`, and wire it into `check`
- [ ] 2.2 Refresh `build-logic` dependency lockfiles for the new suite's configurations; verify `:build-logic:functionalTest` runs (empty) and root `check` still passes
...
- [ ] 5.1 M1 mutation drill — each of the three disarmings makes `:build-logic:functionalTest` fail, then revert: (a) `failOnModification = false`, (b) `onlyIf { false }` on `japicmpApiGate`, (c) remove the gate's `check` wiring
- [ ] 5.2 Full root `check` green; run `openspec validate --strict` on this change; recommend a commit message
