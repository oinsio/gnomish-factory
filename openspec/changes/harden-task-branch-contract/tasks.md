...
- [ ] 0.3 Add glossary entries: branch shape (with the canonical eleven-shape table copied
      ...
      record `Escalated` and `Decision` as banned synonyms for the `Parked`
      and `Answered` shapes
...
- [ ] 5.6 Generalized reaper repair: sweep = `listReady` + `listOpen`; `StalenessMemory`
      ...
      roll `ClaimPending` back and complete `IndexLagging` flips; covers the reap
      sequence's own kill windows (FR19, FR12)
