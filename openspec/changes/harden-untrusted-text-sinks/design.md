...
one table.** `LogText.forConsole` renders ESC as `^[`, C0 other than `\n` and `\t` as
`^X`, DEL as `^?`, C1/bidi/format as `\uXXXX`, `\r` as `\r`; it keeps `\n`
and `\t` as themselves — the two the character table does not name, so the
indentation of a printed stack trace survives — does not flatten `\n`
