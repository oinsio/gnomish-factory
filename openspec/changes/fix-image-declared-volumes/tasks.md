...
- [ ] 2.1 Introduce the `ContainerRunSpec` parameter object (key, image, runtime,
      ...
      fragment is `type=tmpfs` and carries no `source=`/`src=` and no volume name,
...
- [ ] 4.2 Add `ContainerDeclaredVolumesSpec` in `adapters/git` (Docker-gated,
      ...
      from the fixture image, `exec` a write into `/cache` (succeeds as the image's
      non-root user — Docker's default tmpfs mode is `1777`, so add no
      `tmpfs-mode`), assert `/cache` is a tmpfs mount inside the box
...
- [ ] 5.2 `docs/guides/operator-guide-sandbox.md`: a short section stating that
      ...
      in the box, the two alternatives for content that must persist or ship with
      ...
      it.
- [ ] 5.3 M2 gate (non-regression: the grep is already empty today, G2 keeps it so):
