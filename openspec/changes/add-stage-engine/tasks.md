...
- [ ] 2.7 Immutable `TaskState` with transition factories: record unburned round, record quality failure (+1), advance stage (history reset) — FR13, FR14, D4
...
- [ ] 2.9 `TaskOutcome` sealed (Completed / Paused / Escalated / Aborted(failedAt, cause), each with final state) + `EscalationReport` five-kind sealed family — FR10
...
- [ ] 3.2 Check ports: `BuiltinCheckRunner`, `CommandCheckRunner`, `ExternalCheckClient` (single poll → `PollStatus`), `JudgeVoter` (single vote; receives TaskContext) — FR3, D2
...
- [ ] 4.4 Judge voting: sequential single votes (with TaskContext and decisions), majority verdict, failing-vote findings aggregated, per-vote tokens captured — FR3, FR7, NFR-C1
...
- [ ] 5.1 Engine entry `run(definition, context, state, workspace, ports)`: pre-port validation — PipelineMismatch for a stale position, AttemptsExhausted for `attemptsUsed >= limit` on entry — then RunStarted — FR1, FR5, FR9
...
- [ ] 5.4 Persistence ordering: persist → AttemptFinished → next AttemptStarted; persist failure → Aborted(failedAt, cause) with ERROR log — FR11, NFR-O1
- [ ] 5.5 AttemptsExhausted escalation on the last permitted quality failure, carrying the full recorded history — FR5
...
- [ ] 5.8 Advancement: auto → next stage / Completed after last; manual → Paused with position advanced; history resets on advancement — FR8, FR14
- [ ] 5.9 Resume matrix (M3): mid-pipeline, mid-retry (counter and feedback continue), post-pause starts at the next stage — FR9
...
- [ ] 6.1 Event emission across the whole run: all seven events, synchronous, complete keys — FR12
...
- [ ] 7.1 Extend the ArchUnit domain-purity spec to `domain.engine..` with the explicit `org.slf4j` allowance — NFR-O1, D9
- [ ] 7.2 Reentrancy spec: two concurrent runs with independent fakes, identical results, no cross-talk in events or state — NFR-R1
