...
- FR4: verdicts SHALL classify as `Pass` | `Fail(findings, possibly empty)` | `CannotVerify(reason, details)`; a quality failure increments the attempt counter and feeds the non-passing check results (every non-`Pass` result, including `CannotVerify`) of ALL prior attempts of the stage into the next executor request; `CannotVerify` escalates without burning an attempt; adapter exceptions are caught and treated as `CannotVerify` with the stack trace preserved
