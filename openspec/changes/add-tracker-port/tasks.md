...
- [x] 1.1 Value model in `app/port/tracker`: `TaskRef`, `TaskSnapshot`,
...
- [x] 1.2 `Tracker` port interface with the ten v1 operations and doc comments
      carrying traceability links (FR1, D1)
- [x] 1.3 `InstanceId` composite generation: name from `factory.instance-name`
      (default `gnomish-factory`) + per-process base36 suffix; spec for
      uniqueness across same-name processes (FR9, D6)
- [x] 1.4 ArchUnit boundary rule: no core class references
      `adapter/tracker/**` types — the take runner compiles against the port
      alone (FR1)

## 2. Contract suite and in-memory reference adapter

- [x] 2.1 Contract property — `listReady` filtering: states, non-task
      artifacts, readiness criterion, no backoff filtering, queue order +
      abort facts (FR4)
- [x] 2.2 Contract property — claim atomicity: concurrent race yields exactly
      one winner; claim is issued only against `Ready` tasks (per 0.2)
      (FR4, NFR-R1)
- [x] 2.3 Contract property — marker round-trip: abort facts readable
      cross-instance; `collectDecisions` empty after an ack until a new human
      reply (FR4)
- [x] 2.4 Contract property — `fetchTask` facts: snapshot fields,
      `Working(holder)`, `AwaitingHuman(reason)`, abort facts; `Gone` for
      closed/missing without exception (FR1, FR2)
- [x] 2.5 In-memory adapter core: task store and the ten port operations,
      no config subsection (FR3)
- [x] 2.6 In-memory test harness: human operations (reply, return, close) and
      deterministic race interleaving hooks (FR3)
- [x] 2.7 Wire in-memory adapter into the contract suite; all properties green
      (FR3, FR4, M1, M2)

## 3. pipeline-config tracker section

- [x] 3.1 Core keys parsing: optional `tracker` section, `type`,
      `abort-threshold` default 3; absent section leaves loading unchanged —
      specs for both (FR17)
- [x] 3.2 Seam validation with located, aggregated errors: unknown `type`, `type`
      without subsection, mismatched subsection; adapter-validator delegation
      hook (FR17, D5)

## 4. GitHub adapter

- [x] 4.1 Build dependencies: WireMock and Resilience4j (neither is present
      yet), lockfile update, dependency-analysis and Spotless gates green
      (NFR-R2, NFR-P1)
- [x] 4.2 `tracker.github` subsection validator: mandatory `api-url`, `repo`,
      `labels` as `{name, color}` with hex validation; errors aggregate with core
      errors (FR17, NFR-S1 token-from-env only)
- [x] 4.3 Canonical id: build/parse `github:owner/repo#42`, host inclusion by
      normalized `api-url` comparison (D7); round-trip spec; sanitize reuse
      unchanged (FR16)
- [x] 4.4 HTTP client core: `java.net.http` + Resilience4j retry, auth header
      from `GNOMISH_GITHUB_TOKEN`; WireMock specs for retry and auth
      (NFR-R2, NFR-S1)
- [x] 4.5 Conditional requests: ETag cache, `If-None-Match`, 304 handled as
      "no change" without consuming rate limit; WireMock specs (NFR-P1)
- [x] 4.6 Label operations: point add/remove state transitions, exclusivity;
      WireMock specs for lost-update avoidance (FR5)
- [x] 4.7 Label provisioning: idempotent create with color+description, no
      recolor of existing, startup failure on unwritable repo names the repo
      (FR5, NFR-R4)
- [x] 4.8 Structural markers: hidden HTML-comment JSON + human line for
      claim/abort/ack/note/report; parse-back spec (FR7, D9, NFR-O1)
- [x] 4.9 Feed: List Issues query (`state=open`, ready label, asc), PR filtering,
      abort-fact enrichment; WireMock specs (FR8)
- [x] 4.10 `fetchTask`: snapshot + state from labels/closure (`state_reason` into
      revocation context) + holder + abort facts; `Gone` for closed/missing
      (FR2, FR5)
- [x] 4.11 Lease claim happy paths: label → claim comment → boundary-anchored
      verify-read → earliest-id winner; WireMock interleaving spec for the
      race (FR6, NFR-R1, D13)
- [x] 4.12 Lease claim failure paths: loser deletes own marker and reports
      `Held(winner)`; unverifiable claim backs out as infrastructure after
      retries; WireMock specs (FR6, NFR-R1, D13)
- [x] 4.13 Decisions and ack: `collectDecisions` after last ack (`since` +
      boundary anchor), `acknowledgeDecision` (FR12)
- [x] 4.14 State writes: `park`/`finish`/`release`/`recordAbort`/`postNote`
      comment+label operations (FR14, FR18)
- [x] 4.15 Foreign-repo check with rename-redirect tolerance (WARN on redirect
      match, refusal otherwise); WireMock spec (FR16, D8)
- [x] 4.16 Run the shared contract suite against the WireMock-backed GitHub
      adapter; zero exemptions (FR4, M1, M2)

## 5. Take runner and CLI

- [x] 5.1 `TakeResult` model and outcome mapping: Completed→`finish`,
...
- [x] 5.2 Snapshot at first claim into `TaskContext`/`task.json`; resume does not
...
- [x] 5.5 Revocation: `AttemptPersistence` decorator round-boundary check +
      control exception; salvage commit, best-effort push, stop note, release
      (FR15, D2)
- [x] 5.6 Resume wiring over the existing git protocol: branch locate/narrow
...
- [x] 5.7 Decision collection at resume claim and ack-before-acting;
      `DecisionNeeded` with no pending reply parks again restating the
      question (FR12, FR13)
- [x] 5.8 Escalation exit: park with report + return-path message ("reply and
      move the task back to ready"), identical with and without a TTY — no
      in-run wait (FR13, D12, UX3)
- [x] 5.9 Explicit-mode disposition matrix incl. mandate overrides, refusal of
      parked tasks naming the return path, and refusal messages naming
      holder/state (FR9, UX2)
- [x] 5.10 Bare auto mode: eligible head claim, race-loss fall-through, one task,
      clean empty-queue exit (FR10)
- [x] 5.11 Final report rendered from `StatusReport` (stages/attempts/usage/
      branch/wall time) for `finish` (FR18, D11)
- [x] 5.12 Exit-code mapping (D16): `TakeResult` → process exit code
      (0/1/2/3/10/11/12/13/14/15), specs for every family (FR9, FR10, FR15)
- [x] 5.13 `TakeCommand` CLI surface: subcommand registration/dispatch, argument
      parser, flag matrix and validation (no `--mode`, no ad-hoc group, no
      `--from-stage`, bare form rejects `--base`) (FR9, D4)
- [x] 5.14 Short-ref expansion (`42`, `#42` via the binding) and `run`-matrix
      regression: spec asserts existing `run` validation specs unchanged
      (FR9, D4)
- [x] 5.15 Spring wiring: adapter selection by `tracker.type`, factory config
...
- [x] 5.16 MDC canonical task id on all take lifecycle logs, joining the
      existing stage/attempt keys (NFR-O1)
- [x] 5.17 Credentials isolation (D17): adapter registration seam declares
...
- [x] 6.1 Lifecycle integration spec A: ready → claim → work → delivered with
...
- [x] 6.2 Lifecycle integration spec B: escalate → human reply + return to
      ready → cross-instance resume → delivered; second instance shares
      nothing local with the first (M3, NFR-R3)
- [x] 6.3 Revocation integration spec: close-under-work → salvage commit, push,
      stop note, release; branch and worktree kept (FR15)
- [x] 6.4 Abort integration spec: abort → backoff-hidden from bare feed →
      fuse trip at K parks with full abort history (FR14, FR10)
- [x] 6.5 Coverage + PIT over new production code to targets; justify any
      boundary exemptions (M4)

## 7. Documentation

- [x] 7.1 Operator guide `docs/operator-guide.md` per its spec requirement
      (config layers, labels, decisions/ack, snapshot, escape hatch, fork
      warning, CLI reference incl. the D16 exit-code table) (FR19)
- [x] 7.2 Projects v2 section + reference bridge workflow
      `docs/examples/board-bridge.yml` (cron `gh` script: column → ready label)
      (FR19, D14)
- [x] 7.3 Adapter author guide `docs/adapter-author-guide.md` per its spec
      requirement (model, port semantics, contract suite as law, Redmine sketch,
      config subsection ownership, known limitations) (FR19)
- [x] 7.4 README/CLAUDE.md touch-up: `gnomish take` in the CLI surface, tracker
