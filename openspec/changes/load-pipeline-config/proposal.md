...
- ADDED: resolution of autonomy limits (attempts, budgets) from `config.yaml` defaults with per-stage overrides
...
- FR4: artifact references SHALL be by `id` — each output declares a stable `id`; an internal input references the `id` of an output produced by an **earlier** stage in the pipeline order; a `source`/`external` input needs no producer
...
- FR6: semantic validation SHALL enforce: unique stage names; every `pipeline.yaml` stage has a `stages/<name>/stage.yaml`; no stage directory is absent from `pipeline.yaml` (no dangling definitions); every internal input is satisfied by an earlier stage's output; every referenced file (`instructions.md`, acceptance-criteria files for `judge` checks) exists
- FR7: autonomy limits SHALL be resolved from `config.yaml` defaults with per-stage overrides taking precedence
...
- FR9: a `schemaVersion` SHALL be recognized; an unknown or unsupported version SHALL be a validation error
...
- FR11: local (catalog-free) sanity validation SHALL apply to the mechanism and check configs: the executor `model` is required and non-blank when the executor type needs one, and `settings` is carried as an opaque pass-through map (validated as a well-formed mapping, not by key/value/range); an `external` check SHALL have a positive `interval`, a positive `timeout`, and `interval ≤ timeout`, and a non-blank check identifier; a `judge` check SHALL have `votes ≥ 1` and odd. Target-liveness validation (whether a CI check name exists, whether a model is real, whether judge criteria are gradeable) is explicitly deferred (NG7)
