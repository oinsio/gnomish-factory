...
- [x] 1.1 Extend `DockerCommands.inspectContainerState` to
...
- [x] 1.2 Wrap the `ExecHandle` returned by the container adapter's `exec()` so an exit
...
- [x] 1.3 Update `Supervision`'s exit-137 javadoc note to mention that the container
...
- [x] 2.1 Embed `FactoryDockerLabels.containerName(key)` in
...
- [x] 2.2 Embed `FactoryDockerLabels.guardName(key)` in the `EgressGuard`
...
- [x] 2.3 Log one INFO from `ContainerEnvironmentKeeper.stopKeeping` naming the kept
...
- [x] 2.4 Verify by review of the touched messages that only object names and runtime
...
- [x] 3.1 In `SelfCheckedEnvironment.materialize`, catch `SelfCheckFailedException`, stop
...
- [x] 3.2 In `SandboxCheckEnvironmentSource`, skip the dispose-on-materialize-failure when
...
- [x] 3.3 Pin by spec that `FreshJudgeEnvironments` keeps a box whose materialize failed
...
- [x] 3.4 Add a spec asserting the kept-on-failed-self-check box is enumerated by the
...
- [x] 4.1 Run `./gradlew :sandbox:docker:check :adapters:check :adapters:agent:check`
...
- [x] 4.2 Grep-verify traceability: every FR/NFR/UX of the proposal is referenced by at
  least one spec, code javadoc, or test touched by this change (`traceability.md`
  verification rule)
- [x] 4.3 Run `openspec validate polish-sandbox-forensics --strict` and verify it passes
