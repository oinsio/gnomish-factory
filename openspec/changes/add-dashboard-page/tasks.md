...
- [ ] 1.1 Snapshot reader: parse `snapshot.json` into a view model;
      ...
      fixtures — FR3, FR4
- [ ] 1.2 Alert-condition evaluation over the snapshot view model —
      ...
      — FR4 (design D3)
- [ ] 1.3 Ledger aggregator: read the last N daily files, sum
...
- [ ] 2.1 Extract the board fetch+build composition from `BoardCommand`
...
- [ ] 3.2 Self-containment and content-boundary spec: scan rendered
      ...
      M1
- [ ] 3.3 Staleness banner (`--watch` pages only): bake `generatedAt` +
...
- [ ] 3.4 Alert-condition flags rendered visibly (section highlight);
      spec per flagged condition — FR4, UX3

## 4. CLI surface and watch loop (FR1, FR7, FR9, NFR-R1, NFR-R2)

- [ ] 4.1 Add `DASHBOARD` to `Subcommand` and `SubcommandDispatch`;
      ...
      spec — FR1
- [ ] 4.2 Atomic write: temp + rename per render; spec observing only
      complete files at the target path — NFR-R2
- [ ] 4.3 One-shot mode: single render, exit zero even with degraded
      sections; spec for fresh-install and tracker-outage renders — FR3,
      FR7
- [ ] 4.4 Watch loop: render cadence with baked meta-refresh; board
...
- [ ] 5.1 Operator guide: dashboard section — wall-display recipe
