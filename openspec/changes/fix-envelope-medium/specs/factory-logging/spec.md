# factory-logging — delta for fix-envelope-medium

Layered on the factory-logging capability as modified by
`add-subprocess-access-log` (sequenced before this change). That delta
predates the syncs of `harden-untrusted-text-sinks`, `split-logtext-leaves`
and `type-untrusted-text`, so the text below is the capability's current
requirement text — which already carries that delta's findings-sanitizer
funnel and local-string gate — plus this change's one addition: the
output-bounded excerpt. What must survive a hand merge, in either order: the
sink layer and its idempotence, the type-level gate and the capture-point
scan, the findings-funnel rule, and the excerpt bound.
