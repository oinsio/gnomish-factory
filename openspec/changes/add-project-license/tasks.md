...
- [ ] 1.1 Write `NOTICE` at the repository root with exactly the FR2 content: product name,
      ...
      returns 2.
- [ ] 1.2 Write `build-logic/src/main/groovy/distribution-terms-conventions.gradle` (FR3,
...
- [ ] 2.1 Add the jk1 plugin (`com.github.jk1.dependency-license-report`, 3.1.4) to
      ...
      untouched modules.
- [ ] 2.2 Write `config/allowed-licenses.json` (FR5, NFR-R2, D4): `moduleLicense` rules
...
- [ ] 3.1 Write `.github/workflows/license-gate.yml` (FR7, FR1, FR3, NFR-S1, NFR-C1, NFR-O2)
      ...
      `gh workflow view` after push) accepts the file, and the job is green on this branch.
- [ ] 3.2 Draft `CLA.md` from the Apache Individual Contributor License Agreement with the
      ...
      CONTRIBUTING link it, and the signing sentence is byte-identical to the one in
      `cla.yml`; mark the approval as the human's action in 6.3.
- [ ] 3.3 Write `.github/workflows/cla.yml` with `contributor-assistant/github-action@v2.6.1`
      (FR10, NFR-S1, NFR-C1, UX2, D7): triggers `pull_request_target` (`opened`,
      `synchronize`) and `issue_comment` (`created`); a job `if:` admitting every
      `pull_request_target` event and only those comments whose body is `recheck` or the
      signing sentence; workflow-level permissions exactly `contents: write`,
      ...
      `github-actions[bot]`; `custom-notsigned-prcomment` linking `CLA.md` and quoting the
      signing sentence, `custom-pr-sign-comment` = the signing sentence; no checkout step.
      Verify: `actionlint` accepts it, `grep -c "actions/checkout"` returns 0, the signing
      sentence equals the one in `CLA.md` and `CONTRIBUTING.md` (grep all three); on the
...
- [ ] 3.4 Write `CONTRIBUTING.md` (FR10, FR13, UX2, UX4): first paragraph — pull requests
      ...
      paragraph, the signing sentence equals the one in `cla.yml`, and every relative link
...
- [ ] 4.4 In the "CI" paragraph of `docs/guides/developer-guide.md`, add the license job
...
- [ ] 4.6 Add a registry pointer to `docs/guides/adapter-author-guide.md` (FR14): one
      ...
      shows both.
- [ ] 4.7 Add two glossary entries to `docs/glossary.md` in the plugin-contract context
...
- [ ] 6.1 Create the empty `cla-signatures` branch in the repository and leave it out of
      branch protection (D7). Verify: `git ls-remote --heads origin cla-signatures` lists it
      before `cla.yml` is merged.
- [ ] 6.2 Create the `plugin-announcement` label in the repository (D9). Verify: `gh label
      list` shows it before the issue form is merged.
- [ ] 6.3 Review and approve the `CLA.md` wording (Q1), open one pull request from an
...
- [ ] 6.4 Decide whether the registry starts seeded with the in-repo sample row or empty
