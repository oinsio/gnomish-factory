...
- [ ] 1.1 Add `app.port.check.AttemptCommitWorkspace` interface
...
- [ ] 1.2 Move `app.findings.FindingsSanitizer` file-and-FQN from
...
- [ ] 1.3 Document the deliberate `app.findings` split package in both
  modules' `package-info.java` (D3 risk note)

## 2. Retarget the engine record

- [ ] 2.1 Rename the `:application` record to `RecordedAttemptCommitWorkspace`
...
- [ ] 3.3 Remove `implementation project(':application')` from
...
- [ ] 3.4 Spec/gate: `./gradlew :adapters:github:check` green with the edge
...
- [ ] 4.1 Implement the sample `CheckClientFactory` (provider `"sample"`,
  ...
  (FR4, G1, D4)
- [ ] 4.2 Confirm the enforcement is the build, not a spec: the sample module
...
- [ ] 5.1 Run `updateApiCompatibilityBaseline`; review the baseline diff is
  additive-only for pre-existing types; japicmp gate green (FR5, D5)
- [ ] 5.2 Full `./gradlew check` (JaCoCo, PIT per moved scope, ArchUnit,
  dependency-analysis, Spotless) green (M1)
- [ ] 5.3 Verify traceability: every FR/NFR/G of this change grep-resolves to
