...
- **FR4**: On observing a `finished = true` task in the Ready feed, the factory MUST decline it via a tracker-port operation that (a) posts a human-readable comment stating the task is already finished and directing the human to open a new task or bug, and (b) restores the task's terminal status so it leaves the Ready feed.
