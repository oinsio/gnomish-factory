...
The v1 contract is amended in place (design D12: no v1 document has ever been released; the versioning policy governs released contracts): usage moves to per-model token maps, `executing` activity gains live executor detail, and the reference file is regenerated.
