...
**D10 — Startup test strategy.** FR2 ("headless worker boots and exits 0") is specified at context level: a `spock-spring` specification boots the real headless context and asserts the properties bean; process-level exit-code testing is deliberately out of scope for unit gates.
