...
- [x] 1.1 `ReadyTask` gains the `returned` fact in the port value model, with
      doc-comment traceability; unit specs for the value object (FR7)
- [x] 1.2 Contract property — returned fact: false for never-claimed tasks,
      true after park-and-return, true after stale-claim removal; `listOpen`
      size doubles as the open-front count (FR6, FR7, M1)
- [x] 1.3 In-memory adapter derives `returned` from its recorded history;
      contract suite green on the reference (FR7)
- [x] 1.4 GitHub adapter derives `returned` from thread markers (park report,
...
- [x] 4.1 Slot ledger: `Semaphore(N)`, permit acquired before claim, released
...
- [x] 8.1 Operator guide — autonomous operation: serve/batch/drain reference
...
- [x] 8.2 Operator guide — autonomy gate and CI hygiene: ready-label access
...
- [x] 9.1 Traceability sweep: every FR/NFR/UX of the proposal grep-resolves
