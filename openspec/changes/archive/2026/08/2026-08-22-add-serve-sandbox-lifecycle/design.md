...
Project identity: a fourth label scoping all listing — chosen derivation is a stable digest of the configured origin remote URL with an explicit config-key override (`factory.sandbox.project-id`). Without it, project A's tracker honestly answers "no claim" for project B's keys and A would stop B's live boxes. A clone with no `origin` falls back to a digest of its own canonical absolute path, never to a shared constant, which would put every origin-less project on a host into one sweep scope — the exact reach the label exists to prevent; per-clone scoping errs the safe way, two checkouts of one origin-less repo simply ignoring each other. The override is rejected unless it matches `[A-Za-z0-9._-]+`, without echoing the offending value: the label set is rendered and read back as `k1=v1,k2=v2`, so a comma or an equals sign in the value could forge a second label pair and strip an object of its ownership mode.

### D6: One policy component, listener-shaped sinks

The matrix is evaluated by a single component in `sandbox/docker`, emitting one verdict event per object (category, object, role, ownership mode, task key, reason, age) through a listener seam; the mode field lets sinks separate a routine manual age-stop from a dead-instance symptom (dashboard alerts key on `tracked` stopped-orphans only). Sinks: serve → sweeper vitals + ledger lines; run/take → structured SLF4J lines with identical category/field names, closed by a per-invocation summary line. The summary stays in the log rather than in take's finish report: a finish report describes ONE task, while the sweep is project-wide and mostly concerns objects of other tasks. This kills the "serve is safe, run still snapshot-sweeps" divergence structurally (proposal FR9), and makes the policy unit-testable as an event stream. Snapshot/ledger stay daemon-only (proposal NG4): the ledger is single-writer by design and a one-shot process's snapshot is stale at exit. Known gap, accepted: a zombie stopped by a one-shot `take` is absent from the daemon ledger; the kept inventory reflects the *effect* on the next tick, authorship stays in take's log (proposal Q3).
...
### D8: One take-resume routing table, two resume mechanics behind a seam

Adopting container mode in `take` (FR1) first produced a mirrored class per host resume class — `TakeContainerDispositionResume`, `TakeContainerDecisionResume`, `TakeContainerReconcile` beside `TakeDispositionResume`, `TakeDecisionResume`, `TakeReconcile` — whose javadoc claimed to mirror the host routing table "exactly". It did not: the `Completed` → deferred-finish branch was never mirrored, so a container-mode task that delivered its work while its tracker finish never landed could not be resumed by ref at all. That is the textbook failure mode of parallel hierarchies — a branch added to one side and forgotten on the other — and copying a fourth branch across would deepen it.

Chosen: the routing table lives **once**. `ResumedBranch` is the common view of a loaded task branch (taskId, context, recorded outcome, last escalation, branch name, tracker-write-pending marker) that both bundles already expose field-for-field; `ResumeMechanics<B extends ResumedBranch>` is the seam carrying everything that genuinely differs — load the branch, read its final state, clear the tracker-write marker, resume with or without a decision. `TakeDispositionResume` and `TakeDecisionResume` become generic over `B` and are shared by both modes; the three container mirrors are deleted. Genericizing is what `TakeContainerDecisionResume`'s own javadoc named as the missing precondition for sharing ("no common supertype to dispatch through without genericizing both").

*Rationale:* the routing table is domain policy that FR1 requires to be identical in both modes, while worktree-materialize-and-salvage versus reattach-box-and-salvage-in-box is mechanism. Keeping policy in one place and mechanism behind a port-shaped seam is the same rule this codebase already applies to trackers and executors, and it makes the routing predicates contract-testable once over a fake mechanics instead of twice over mirrored ones.

*Alternative rejected:* mirroring `deliverCompleted` into `TakeContainerReconcile` as a fourth branch — the smallest diff, but it re-buys the same drift for every future branch, and the `Completed` reconcile needs no sandbox at all (it reads the delivered state from branch history through `TaskBranchGit` and posts the finish through the tracker), so a container-specific copy would have been a copy with nothing container-specific in it.

Detection of the delivered-and-cleaned shape stays **inside** each mechanics, not hoisted into the shared table: the `Completed` cleanup commit (FR15 of add-git-workflow) strips `.gnomish-task/` from the tip, and how that absence surfaces is mechanism-specific — a `NoSuchFileException` reading the materialized worktree on host, a `MissingObjectException` reading bare objects on container. Each mechanics translates its own signal into the one shared vocabulary (`loadBranch` returns null), so the shared table decides policy on a mode-independent fact. The alternative — one pre-check through `TaskBranchGit` before either mechanics runs — was rejected because on host it would read the pre-reconcile local tip, which can legitimately lag the delivered tip that the worktree read reconciles to first.
...
- [The shared routing table now has two implementations to keep honest] → the seam is five methods wide and every one of them is exercised by both a host and a container spec; a mode that forgets one fails to compile rather than silently skipping a branch.
...
- [Label schema becomes a compatibility surface] → labels are versioned by presence, but only *within* the scoped listing: an object carrying the project label yet no mode label matches no mode and is treated as `tracked` with age protection. An object predating the PROJECT label is invisible to the sweep entirely — project scoping is an AND filter at listing time (FR8), and a second, unscoped pass is not the answer: a label-less object carries no project identity, so sweeping it would be exactly the "touching another project's objects on a shared host" that FR8 forbids. Such objects are cleaned up manually, once; documented in the operator guide.
...
1. Land labels + policy component + entry-point wiring behind the same release; the sweep understands objects carrying the project label but no mode label conservatively (`tracked`, age-protected).
2. First daemon start after upgrade: the startup tick classifies pre-existing *project-labelled* objects; nothing running is disposed, so a mixed-version host degrades safely. Objects created before the project label exists are outside every listing and are removed once by hand — the operator guide carries the command.
