...
- [x] 1.1 Extend the port contract suite with title-propagation properties
      for `listReady` and `listOpen` (red first, TDD) — FR7
- [x] 1.2 Add the `title` component to `ReadyTask` and `OpenTask`;
      compiler-driven sweep of all construction sites (core, fixtures,
      existing specs) — FR7
- [x] 1.3 In-memory adapter: map `title` from the stored snapshot in both
      list operations; contract suite green — FR7
- [x] 1.4 GitHub adapter: extend the shared issue-feed parser to retain
      `number` + `title` pairs; consume in `GithubFeedQuery` and
      `GithubOpenQuery`; contract suite green — FR7
- [x] 1.5 WireMock spec asserting the two list calls issue no additional
      issue-detail requests after enrichment — NFR-P1, M2

## 2. Board model and composition (FR2–FR5, NFR-P1)

- [x] 2.1 Spock spec + immutable `BoardModel` (three columns, summary
...
- [x] 2.2 Eligibility annotation mirroring the feed predicate in precedence
...
- [x] 2.3 Summary counts (queued / eligible now / ineligible by reason, each
...
- [x] 3.1 Add `BOARD` to `Subcommand` and route it in
      `SubcommandDispatch`; parser spec for the token — FR1 (design D1)
- [x] 3.2 `BoardCommand` + arguments parser (`--dir` default current
...
- [x] 3.3 Tracker-outage handling: one clear error line, non-zero exit,
      no board-specific retry loop; spec with a failing tracker — NFR-R1

## 4. Rendering (FR6, NFR-O1, UX4)

- [x] 4.1 Text renderer (sibling of `TaskListRenderer`): three columns,
...
- [x] 4.2 JSON mapper (sibling of `StatusReportJsonMapper`): `"version":
...
- [x] 4.3 Reference JSON fixture (`board-v1.reference.json`) + spec
      pinning the document shape; text/JSON agreement spec (same model,
      same facts on both surfaces) — FR6, UX4, M1

## 5. Docs and verification

- [x] 5.1 Operator guide (`docs/operator-guide.md`): board section (columns,
