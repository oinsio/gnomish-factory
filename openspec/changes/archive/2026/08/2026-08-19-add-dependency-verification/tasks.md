...
- [x] 1.1 Generate `gradle/verification-metadata.xml` with sha256 checksums for
...
- [x] 1.2 Add the trusted-artifact entries for `sources` / `javadoc`
...
- [x] 1.3 Verify idempotence: run the combined regeneration command
...
- [x] 2.1 Confirm wrapper validation is already enforced: `ci.yml` uses
  `setup-gradle` with `validate-wrappers: true`, and no other workflow
  executes the wrapper — no new step (FR5, D3).
- [x] 2.2 Audit all workflows: no flag or property disables dependency
  verification anywhere in CI (FR5, D5).

## 3. Fail-closed verification (M1 tamper test)

- [x] 3.1 Tamper test: corrupt one cached artifact (or add an unlisted
...
- [x] 3.2 Confirm the failure surface points to the fix: the documented
  regeneration command is reachable from the failure docs (FR2, UX1).

## 4. Docs + update flow

- [x] 4.1 Document, next to the Dependabot config reference and the existing
  ...
  (FR3, FR4, NFR-S1, D2).
- [x] 4.2 Prove the flow on one real bump (a live Dependabot PR or a manual
  version bump run through the same steps) (M2).

## 5. Traceability + handoff

- [x] 5.1 Verify every FR/NFR/UX of `add-dependency-verification` has an
  implementing entity (metadata file, CI step, docs, or recorded test) per
  `traceability.md`.
- [x] 5.2 Recommend a Conventional Commits message referencing this change
