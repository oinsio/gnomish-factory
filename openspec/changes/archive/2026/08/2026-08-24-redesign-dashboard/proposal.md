...
- **ADDED**: relative time rendering ("N min ago") with a 1-second client
...
- FR7: tokens SHALL render per model as a stacked bar (cacheRead /
  ...
  cache share; zero cache renders "cache not in use". Spend SHALL NOT use
...
- FR9: counts ≥ 1000 SHALL be formatted compactly server-side in Java
  (three significant digits, trailing zeros dropped — `25.6K`, `4.79M`,
  `5M`), exact value in `title`; numeric cells use tabular numerals and the
  mono stack. Percentages are integers.
...
- UX2: relative ages use short forms that need no plural agreement
  (`N s ago`, `N min ago`); hovering reveals the exact instant.
