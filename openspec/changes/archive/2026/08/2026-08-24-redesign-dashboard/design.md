...
`<body data-generated-at>` and the stale threshold (three times the render
cadence) from `data-stale-after`, keeping the script a static resource with
no templating inside it — the script holds only a fallback threshold pinned
to the shipped cadence by a spec. The watch/one-shot split rides the same
channel:
...
value (NFR-R2). Two conscious carve-outs: the freshness strip's own
`<time>` is dropped from the ticking list (the strip owns its whole
sentence; the script lifts the ISO text onto the strip's `title` first),
and an instant the page presents as a future deadline — a ready row's
backoff — stays prose, since a relative form measures elapsed time.
*Rationale:* one rendering path that works without JS; JS
...
renderer as plain methods separate from string assembly (shipped as
`DashboardCompactNumber` and `DashboardPercentage` next to the existing
`DashboardHtmlFormatter`), with Spock specs
...
**D7 — `dashboard.css` keeps its 200-line exemption; the block specs do
not.** The file-size cap of `.claude/rules/process-invariants.md` allows
200 lines "only when splitting would hurt clarity". The multi-block specs
did not qualify: `DashboardBoardBlocksSpec` and `DashboardHistoryBlocksSpec`
each covered two independent blocks, and the renderer spec carried the
status card and the hygiene block besides the skeleton, so they are split
one spec per block — status card, attention, in-progress, their joint
degradation, outcomes, tokens, hygiene — with the shared render call and
fixtures in `DashboardBoardFixtures` / `DashboardHistoryFixtures` /
`AlertSnapshotFixtures` / `DashboardPageMarkup`. This closes the
"one capability per spec file" rule of `.claude/rules/testing.md` as well.
`dashboard.css` (274) stays whole: it is one prescriptive asset read
top-to-bottom, its `:root` token block governs every rule below it (NFR-O1),
and splitting it would either duplicate the tokens or force a second
classpath resource plus a concatenation step in `DashboardResources` —
weakening the fail-fast-on-missing-resource guarantee (FR10) to save ~75
lines. `DashboardWatchLoopSpec` (225) is pre-existing from
`add-dashboard-page` (214 before this change) and out of scope here.
*Rationale:* the cap exists to protect AI context quality; a stylesheet read
as one unit and a spec covering one block both serve that, a spec covering
two blocks does not.
