...
- [x] 1.1 TDD (red first): extend `SanitizerPairEquivalenceSpec`'s adversarial corpus
...
- [x] 1.2 Extend the one character-class table in `LogText` and mirror it in
...
- [x] 1.3 TDD (red first): `LogTextConsoleSpec` pinning `LogText.forConsole` (design D6,
      FR5): ESC → `^[`, other C0 → `^X`, DEL → `^?`, C1/bidi/format → `\uXXXX`, `\r` →
      literal `\r`; `\n` preserved; no cap; plain text byte-identical. Verify: red.
- [x] 1.4 Implement `LogText.forConsole`. Verify: 1.3 green.
- [x] 1.5 TDD (red first): `LogTextRecordCapSpec` pinning `LogText.capRecord` (design
...
- [x] 1.6 Idempotence spec (FR2, design D2): data-driven over the whole corpus,
...
- [x] 2.1 TDD (red first): `SafeMessageConverterSpec` (FR1, design D1): a Logback
...
- [x] 2.2 TDD (red first): `SafeThrowableConverterSpec` (FR3, design D3): a thrown-and-
...
- [x] 2.3 TDD (red first): `SafeMdcConverterSpec` (FR4, design D4): a hostile `stage`
      value renders single-line and inert; a missing key renders as today (empty).
      Verify: red.
- [x] 2.4 Implement the three `ClassicConverter` subclasses in `bootstrap/…/logging/`,
...
- [x] 2.5 TDD (red first): NFR-R1 in each converter spec — a converter whose primitive
      throws (injected failure) writes the bounded placeholder record and the appender
      keeps appending. Verify: red, then green.
- [x] 2.6 Register the three `<conversionRule>`s and rewrite `GNOMISH_LOG_PATTERN` to use
...
- [x] 2.7 Converter-source scan spec (single-owner table, row 3): the converter sources
      contain no escape-character literal or `\p{Cc}`-class pattern. Verify: green;
      seeded violation detected.
- [x] 2.8 Run `LogContractGateSpec` and the `checkLogExpectationGate` task (NFR-O2).
      Verify: both green unchanged — no `[GFnnn]` head moved.

## 3. The console owner in `:application`

- [x] 3.1 TDD (red first): `SystemConsoleIOSpec` (FR5): `print` applies `forConsole`
      ...
      Verify: red.
- [x] 3.2 Add `ConsoleIO.printMachine(String)`; implement in `SystemConsoleIO`; update
...
- [x] 3.3 Reroute the 25 direct print sites per design D5's table — `StatusCommand`
...
- [x] 3.4 TDD (red first): `ConsoleOwnerGateSpec` in `:bootstrap` (FR6, single-owner
...
- [x] 3.5 `AbortHandler` (design D7): pass the original throwable as the trailing
...
- [x] 3.6 `TakeOutcomeMapper:95` (design D8, FR7): build the report through the
...
- [x] 4.1 Amend `docs/adr/0004-logging-policy.md` (FR8): replace "Known limit: untrusted
...
- [x] 4.2 Update `.claude/rules/logging.md`: the "Known limit" subsection points at the
...
- [x] 4.3 `docs/glossary.md` (Observability): add *operator console*; extend *log text
      sanitization* with the sink layer sentence. Verify: entries present; no banned
      synonym introduced.

## 5. End-to-end invariant and module gates

- [x] 5.1 `UntrustedTextSinkInvariantSpec` in `:bootstrap` (NFR-S1, design D9): load the
...
- [x] 5.2 Console half of the invariant (NFR-S2): the corpus through `SystemConsoleIO`
      over a captured stream — human path inert with line breaks preserved, machine path
      byte-identical. Verify: green (M1).
- [x] 5.3 Module gates: `./gradlew :logtext:check :bootstrap:check :application:check`
...
- [x] 5.4 Old-way sweep report (implementation.md): `grep -rn "System\.\(out\|err\)\.print"
