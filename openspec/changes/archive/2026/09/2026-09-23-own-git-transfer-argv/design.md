...
The typed entry has one residual path: `GitTransfer` is a public record, so its canonical
constructor is public by the language rule, and `new GitTransfer(argv, environment)` with an
argv read from data spells no literal for D7's scan and reaches `run(Path, GitTransfer)`,
which the refusal never inspects. `GitTransferConstructionBoundarySpec` in `:bootstrap`
closes it for production: no `src/main` outside `:gittransfer` constructs the value
(`new GitTransfer(`, `GitTransfer::new`), with one listed fixture —
`TransferAdversaryFixture.seedTransfer`, which substitutes the branch parameter as the seed
script's `"$1"` does. *Alternative rejected:* a sealed interface over a package-private
record with construction only in the owner — it closes the path by type, but it also removes
the hostile values the renderer's and the runner's refusal specs hand in, which is the
evidence those refusals work.
...
consume it. One declared pair is touched: `TakeResumeBootstrap ↔ TakeContainerResumeBootstrap`
— the worktree end gains a `BranchLocation.Refused` arm that throws
`BranchLocationRefusedException` (FR5). No mirrored edit is needed: the pair's synchronized
invariant is hardening the clone, reconciling the remote on resume-start, and the resume
bundle's shape, none of which changes; the container end never classifies the location itself
— it calls `ensureLocalTaskBranch`, whose `ContainerResumeBranch` already throws
`BranchLocationRefusedException` on `Refused`. One neighbour is recorded so it does not become an
...
One **declared pair** is created (review of 2026-09-23): `AdversarialGitConfig` exists twice,
in `build-logic` (sets `GIT_CONFIG_GLOBAL` and the file path on every forked test JVM) and in
`:test-fixtures` (reads them back and owns the keys' meaning). No classpath joins them — the
build cannot load a test fixture — so the variable name and the relative path are spelled at
both ends. Both carry the `Kept in sync with` marker and the pair is listed in the
"no shared classpath" table of `manual-sync-pairs.md`; `AdversarialGitConfigSpec` is the
mechanical check, since the global configuration git lists must equal the resource.

### Single-owner mechanisms

| Owner                                          | Value (type)                                                                                                                          | Consumers                                                                                                                                                                                                                                                    | Old way removed                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Enforced by                                                                                                                                                                                                                                                                                        |
|------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `GitTransfer.fetch/clone` (`:gittransfer`)     | `GitTransfer` (argv + environment)                                                                                                    | `BaseRefresh.fetchBranch`, `TagBaseFetch.fetch`, `CommitBaseFetch.fetch`, `TaskBranchLocator.locate`, `ReplicaPairReconciler` (all `adapters:git`); `ContainerHarvestFetch.fetch` (`adapters:git`); `DockerSeedCloneCommand.seedClone` (`sandbox:docker`)    | `NarrowFetch` — deleted (D9); the literal argv in `ContainerHarvestFetch.fetch` — deleted; the `git clone ...` line in `SEED_SCRIPT` — replaced by the rendered value. Sweep pattern: `"fetch"`, `"clone"`, `git clone`, `git fetch` across every `src/main`. Exemptions: none                                                                                                                                                                                        | `GitProcessRunner.run(Path, GitTransfer)` is the only entry that runs a transfer; `run(Path, String...)` throws on a transfer subcommand (D5); `GitTransferBoundarySpec` scan with reach assertion (D7); `GitTransferConstructionBoundarySpec` — no production construction outside the owner (D5) |
