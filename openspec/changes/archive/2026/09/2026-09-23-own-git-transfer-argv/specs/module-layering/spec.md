...
Written over `openspec/specs/module-layering/spec.md` as it stands after the
`split-logtext-leaves` sync (2026-09-21): both requirements below are the
stable text plus the `:gittransfer` sentences and the scenarios that name it,
so a replace-only sync keeps every stable sentence — the transitive-reach
paragraph, the "declare the edge in the change that uses it" rule, the
`FR11` trailer of `split-logtext-leaves`, and the access-log sentences with
their `FR2, FR3` trailer of `add-subprocess-access-log` (sequenced before this
change for these two requirements; its text entered the stable spec through
the `add-base-ref-resolution` sync) included. What this change adds and
what must survive any later merge: the `:gittransfer` sentence in each
requirement, the `:gittransfer` clause of the transitive-reach scenario, and
