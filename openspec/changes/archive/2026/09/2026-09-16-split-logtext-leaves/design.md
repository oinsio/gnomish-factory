...
`LogContractGateSpec` (proposal FR10): a string literal matching `"[GF`
anywhere in a production source — not only inside a log call — fails the build
naming the site. Scanning call text alone would leave the head parked in a
constant, in an exception message, or in a string assembled one statement
above the call unjudged, which is the same escape hatch under another spelling. The gate's present
