...
| `RemoteDefaultBranch.discover` — the one place the repository default branch is established, and the only place a `DefaultBranch` is built | `DefaultBranch` in `DefaultBranchDiscovery.Discovered`, carried by `TrustedTierStartup.StartupLaw` and `TrustedBaseContext` | `ServeCommand`, `TakeCommand` (construct `TrustedBaseContext`), `FreshClaimBaseBinding` (builds the `BaseRefRequest`), `BaseRefResolver`'s `REPOSITORY_DEFAULT_BRANCH` tier                                                                                                                     | the `String defaultBranch` of `Discovered`, `StartupLaw` and `TrustedBaseContext`, through which any caller could stand in `"HEAD"`; the ref-name check that lived in `RemoteDefaultBranch.discovered` moves into the type. Exemption: `BaseRefResolver.LOCAL_HEAD_REF`, the manual tier, which is a different tier and keeps the literal                                                                                        | the `DefaultBranch` parameter type, which refuses `HEAD` as git's own `strbuf_check_branch_ref` does; `DefaultBranchSpec`; `RemoteDefaultBranchSpec`'s HEAD-named-remote feature on real git; `BaseRefResolverSpec`'s "the default-branch tier can never answer the local HEAD". `BaseHeadDefaultBoundarySpec`'s file list no longer covers this half — it was the enforcement that failed |
...
manual `run --resume` never fetches at all — it holds no claim and no
tracker, so it binds from the pinned ref's local tip in the operator's clone
(`ManualResumeLawBinding`), reachable remote or not; the local-tip fallback on
the autonomous path covers a different case, a clone with no `origin` at all. A pinned ref that no longer resolves anywhere (series branch deleted
