...
- [x] 1.1 TDD (red first): write `AbortCauseBudgetSpec` pinning the contract — a cause at
...
- [x] 1.2 Implement `AbortCauseBudget` in `application/.../app/take/` per design D2–D4:
...
- [x] 2.1 TDD (red first): extend `AbortHandlerSpec` — an over-budget cause reaches
...
- [x] 2.2 Wire `AbortCauseBudget.cap` into `AbortHandler.handle` after the ERROR log and
...
- [x] 2.3 Fuse-accounting bound (NFR-R1): add a spec asserting the complete fuse-trip
...
- [x] 3.1 Run `./gradlew :application:check` (Spotless, Error Prone/NullAway, Spock,
      JaCoCo, PIT — mutation gate must stay at 100% for the new class). Verify: BUILD
      SUCCESSFUL with no new PIT survivors.
- [x] 3.2 Cross-check traceability per `.claude/rules/traceability.md`: every FR/NFR of
