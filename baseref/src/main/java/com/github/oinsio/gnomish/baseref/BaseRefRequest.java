 ...
 * names: the ref-shaped inputs would otherwise be adjacent strings, a transposition hazard no
 * compiler catches. Two of the three now carry their own types ({@link BaseDesignator}, {@link
 * DefaultBranch}), and the remaining pair is separated by the non-string components, so a
 * mis-ordered construction does not compile; the request is built once at the funnel.
 *
 ...
 *     is no remote or the caller could not ask. Typed rather than a bare name so this tier can only
 *     be fed a value that really came from a remote — a caller cannot stand in the clone's local
 *     {@code HEAD} under the sentence this tier reports (FR4, FR10, M2)
 */
public record BaseRefRequest(
        ...
        ResolutionMode mode,
        @Nullable DefaultBranch defaultBranch) {
