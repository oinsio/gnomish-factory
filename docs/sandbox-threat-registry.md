...
| 46 | Object poisoning of the operator clone through harvest: a malformed object (a `.gitmodules` with a traversal path, a broken tree, a bad tag) authored in the box enters the factory clone's object database, where later checkouts and tools interpret it | every transfer validates received objects (`fetch.fsckObjects` / `transfer.fsckObjects`, three legacy ignores only); a refusal is a boundary violation of the box with a report naming the object and the message id, never an infrastructure failure | shipped (`own-git-transfer-argv`, ADR 0008) |
| 47 | Ref planting into the operator clone from the box: a tag the gnome creates is auto-followed into `refs/tags/`, `FETCH_HEAD` is rewritten, a planted `.gitmodules` triggers a second fetch against a host of the gnome's choosing, or a URL rewrite in the operator's global configuration redirects the transport | one transfer owner with a deny-by-default set on every source (`--no-tags`, `--no-write-fetch-head`, no submodule recursion by flag and configuration, no prune, `--end-of-options`); the harvest runs under a protocol allowlist of `ext` alone and reads no operator configuration; the operator's `git for-each-ref` differs only in the one task-branch ref | shipped (`own-git-transfer-argv`, ADR 0008) |
| 48 | Operator ref and tag disclosure into the box: the seed clone carries the operator's local tags and other branches into the untrusted working copy, and in git's local-path mode copies objects without validation | the seed clone is built by the transfer owner: `--no-local --no-hardlinks --single-branch --no-tags`, validated objects, protocol allowlist `file` alone, no operator configuration beyond a throwaway `safe.directory` file; the box holds exactly the task branch and no tag; the git version floor 2.45.1 keeps the local-clone hook protections in force | shipped (`own-git-transfer-argv`, ADR 0008) |
