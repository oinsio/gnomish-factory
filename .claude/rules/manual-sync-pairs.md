...
|--------------------------------------------------|------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `adapters/git/.../GitAttemptPersistence`         | `adapters/git/.../EnvironmentAttemptPersistence`                 | attempt commit + state-file write sequence. The tip-resolution half of it is no longer hand-synced: both media resolve through `VerifiedTip` (`harden-logging-observability` FR13), so a blank or failed resolution refuses on both by construction. The denial cursor and denial identity are deliberately environment-side only and are **not** part of the synchronized invariant: host mode has no egress guard, so no denial source exists to mirror (provenance: `fix-denial-attribution-durability`). |
