...
The envelope file names (`GnomishTaskPaths` in `adapters/git` against
`BranchShapeClassifier` in `:domain`, with a third hand-typed spelling in
`:bootstrap`) never earned a row: `fix-envelope-medium` dissolved them into one
owner, `domain.branch.EnvelopePaths`, under a build gate, rather than declaring
a pair that already had three ends.

| End A                                            | End B                                                            | Synchronized invariant                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
|--------------------------------------------------|------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `adapters/git/.../GitAttemptPersistence`         | `adapters/git/.../EnvironmentAttemptPersistence`                 | attempt commit + state-file write sequence — that sequence is now the whole invariant. Two halves have left it, each because a single owner replaced the hand-sync: **tip resolution**, since both media resolve through `VerifiedTip` (`harden-logging-observability` FR13), so a blank or failed resolution refuses on both by construction; and the **failure message**, since both now throw `GitPersistFailedException(…, UntrustedText)` and the exception composes its own message from the carrier, so neither medium can word or neutralize the captured git output differently (provenance: `type-untrusted-text`). The denial cursor and denial identity are deliberately environment-side only and are **not** part of the synchronized invariant: host mode has no egress guard, so no denial source exists to mirror (provenance: `fix-denial-attribution-durability`). |
...
| `build-logic/.../build/AdversarialGitConfig`    | `test-fixtures/.../adapter/git/AdversarialGitConfig`                          | the variable (`GIT_CONFIG_GLOBAL`) and the committed file path (`test-fixtures/src/main/resources/adversarial-gitconfig`) the test build hands git (design D11 of `own-git-transfer-argv`); the build cannot load a test fixture, so both spell the constants. `AdversarialGitConfigSpec` in `:bootstrap` pins the identity: what git lists as global equals the resource. |
